...
def parse_args():
    ...
    parser.add_argument('--prefix', dest='prefix', help='trained model prefix', default=os.path.join(os.getcwd(), 'model', 'ssd_300'), type=str)
    parser.add_argument('--num-class', dest='num_classes', help='number of classes', default=20, type=int)
...
if __name__ == '__main__':
    ...
    if args.network == 'vgg16_reduced':
        net = importlib.import_module("symbol_"+args.network).get_symbol(args.num_classes, args.nms_thresh, args.force_nms)
    else:
        net = importlib.import_module("get_symbol").get_symbol(args.num_classes, args.nms_thresh, args.force_nms, args.network)
