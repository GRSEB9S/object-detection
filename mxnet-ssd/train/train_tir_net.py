...
import importlib
import mxnet as mx
from dataset.iterator_spectral import DetIterSpectral
...
def train_tir_net(network_name, image_set, dataset_path, batch_size, data_shape, mean_rgb, std_rgb, mean_tir, std_tir, resume,
    ...
    # init data iterator
    train_iter = DetIterSpectral(train_imdb,
                            batch_size,
                            data_shape,
                            mean_rgb,
                            std_rgb,
    ...
    if val_imdb:
        val_iter = DetIterSpectral(val_imdb,
                           batch_size,
                           data_shape,
                           mean_rgb,
                           std_rgb,
    ...
    for name in net.list_arguments():
        ...
        if 'loc' in name_split :
            ...
            print name
            continue
        elif name.startswith('fusion') or name in ['rgb', 'tir', 'label']:
            print name
            continue
        elif 'tir' not in name_split:
