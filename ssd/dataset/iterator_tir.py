...
class DetIterTIR(mx.io.DataIter):
    """
    ...
    def __init__(self, imdb, batch_size, data_shape,
                 ...
                 is_train=True,
                 max_crop_trial=50):
        super(DetIterTIR, self).__init__()
        ...
        self._mean_pixel = mx.nd.array([43.8573])
    ...
    def _get_batch(self):
        ...
        """
        batch_data = mx.nd.zeros((self.batch_size, 1, self._data_shape[0], self._data_shape[1]))
        batch_label = []
        for i in range(self.batch_size):
            ...
            img = mx.nd.array(np.load(im_path))
            img = mx.nd.expand_dims(img, axis=2)
        ...
        if self.is_train and self._rand_samplers:
            ...
            if num_rand_crops > 0:
                ...
                if xmin >= 0 and ymin >= 0 and xmax <= width and ymax <= height:
                    ...
                    offset_y = 0 - ymin
                    data_bak = data
                    data = mx.nd.full((new_height, new_width, 1), 44, dtype='uint8')
