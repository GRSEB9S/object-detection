...
class Kaist(Imdb):
    """
    Implementation of Imdb for Kaist Multispectral Dataset
    ...
    """
    def __init__(self, image_set, dataset_path, shuffle=False, is_train=False, mode='rgb'):
        super(Kaist, self).__init__(image_set)
        self.mode = mode
        ...
        self.classes = ['person']
        self.config = {'use_difficult': True, 'comp_id': 'comp4', 'padding': 56}
    ...
    def _load_image_set_index(self, shuffle):
        ...
        """
        image_set_index_file = os.path.join(self.data_path, 'image_sets', self.image_set + '.txt')
    ...
    def image_path_from_index(self, index):
        ...
        if self.mode == 'rgb':
            ...
            image_file = os.path.join(self.data_path, 'images/tir/', name + self.extension)
            assert os.path.exists(image_file), 'Path does not exist: {}'.format(image_file)
            return image_file

        elif self.mode == 'rgb-tir':
            image_file_rgb = os.path.join(self.data_path, 'images/rgb/', name + self.extension)
            image_file_tir = os.path.join(self.data_path, 'images/tir/', name + self.extension)
            assert os.path.exists(image_file_rgb), 'Path does not exist: {}'.format(image_file_rgb)
            assert os.path.exists(image_file_tir), 'Path does not exist: {}'.format(image_file_rgb)
            return [image_file_rgb, image_file_tir]
    ...
    def _load_image_labels(self):
        ...
        labels = []
        for label in temp:
            label = np.lib.pad(label, ((0, self.padding-label.shape[0]), (0,0)), 'constant', constant_values=(-1, -1))
            labels.append(label)
    ...
    def get_result_file_template(self):
        ...
        ----------
            a string template
        """
        res_file_folder = os.path.join(self.dataset_path, 'results')
    ...
    def do_python_eval(self):
        ...
        imageset_file = os.path.join(self.data_path, 'image_sets', self.image_set + '.txt')
