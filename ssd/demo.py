...
def get_detector(net, prefix, epoch, data_shape, mean_pixels, ctx, nms_thresh=0.5, force_nms=True):
    ...
    if net == 'vgg16_reduced':
        net = importlib.import_module("symbol_"+net).get_symbol(len(CLASSES), nms_thresh, force_nms)
    else:
        net = importlib.import_module("get_symbol").get_symbol(len(CLASSES), nms_thresh, force_nms, net)

    detector = Detector(net, prefix + "_" + str(data_shape), epoch, data_shape, mean_pixels, ctx=ctx)
    return detector

def parse_args():
    parser = argparse.ArgumentParser(description='Single-shot detection network demo')
    parser.add_argument('--network', dest='network', type=str, default='vgg16_reduced',
                        choices=['vgg16_reduced', 'resnet', 'squeezenet', 'caffenet'], help='which network to use')
    ...
    parser.add_argument('--dir', dest='dir', nargs='?', help='demo image directory, optional', type=str)
    parser.add_argument('--ext', dest='extension', help='image extension, optional', type=str, nargs='?')
    parser.add_argument('--epoch', dest='epoch', help='epoch of trained model', default=0, type=int)
    ...
    parser.add_argument('--thresh', dest='thresh', type=float, default=0.5, help='object visualize score threshold, default 0.6')
...
if __name__ == '__main__':
    args = parse_args()
    args.prefix += '/' + args.network + '/deployed/deploy_ssd'
    ...
    detector = get_detector(args.network,
                            args.prefix,
                            args.epoch,
                            args.data_shape,
