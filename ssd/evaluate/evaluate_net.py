...
from dataset.piropo import Piropo
...
def evaluate_net(network_name,
    ...
    if dataset == "pascal":
        ...
        print mode
        imdb = Kaist(sets, dataset_path, shuffle=False, is_train=False, mode=mode)
        data_iter = DetIter(imdb, batch_size, data_shape, mean_pixels, rand_samplers=[], rand_mirror=False, is_train=False, shuffle=False, mode=mode)
        sys.path.append(os.path.join(cfg.ROOT_DIR, 'symbol'))
        print 'number of classes: ', imdb.num_classes
